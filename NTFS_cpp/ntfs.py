...
class MFTRecord:
    def __init__(self, data) -> None:
        ...
        standard_info_size = int.from_bytes(
            self.raw_data[standard_info_start + 4 : standard_info_start + 8],
            byteorder="little",
        )
        self.standard_info = {}
        ...
        if data_sig[0] == 64:
            data_start += int.from_bytes(
                self.raw_data[data_start + 4 : data_start + 8], byteorder="little"
            )
    ...
    def __parse_data(self, start):
        ...
        if self.data["resident"]:
            offset = int.from_bytes(
                self.raw_data[start + 0x14 : start + 0x16], byteorder="little"
            )
            self.data["size"] = int.from_bytes(
                self.raw_data[start + 0x10 : start + 0x14], byteorder="little"
            )
            self.data["content"] = self.raw_data[
                start + offset : start + offset + self.data["size"]
            ]
        else:
            ...
            self.data["size"] = int.from_bytes(
                self.raw_data[start + 0x30 : start + 0x38], byteorder="little"
            )
            self.data["cluster_size"] = int.from_bytes(
                self.raw_data[start + 0x41 : start + 0x41 + size], byteorder="little"
            )
            self.data["cluster_offset"] = int.from_bytes(
                self.raw_data[start + 0x41 + size : start + 0x41 + size + offset],
                byteorder="little",
            )
    ...
    def __parse_file_name(self, start):
        ...
        name_length = body[64]
        self.file_name["long_name"] = body[66 : 66 + name_length * 2].decode(
            "utf-16le"
        )  # unicode
    ...
    def __parse_standard_info(self, start):
        ...
        offset = int.from_bytes(
            self.raw_data[start + 20 : start + 21], byteorder="little"
        )
        begin = start + offset
        self.standard_info["created_time"] = as_datetime(
            int.from_bytes(self.raw_data[begin : begin + 8], byteorder="little")
        )
        self.standard_info["last_modified_time"] = as_datetime(
            int.from_bytes(self.raw_data[begin + 8 : begin + 16], byteorder="little")
        )
        self.standard_info["flags"] = NTFSAttribute(
            int.from_bytes(self.raw_data[begin + 32 : begin + 36], byteorder="little")
            & 0xFFFF
        )
...
class MFTFile:
    def __init__(self, data: bytes) -> None:
        ...
        self.file_name_len = int.from_bytes(
            self.raw_data[0x9C:0xA0], byteorder="little"
        )
        ...
        self.num_sector = (
            int.from_bytes(self.raw_data[0x118:0x120], byteorder="little") + 1
        ) * 8
        del self.raw_data
...
class NTFS:
    ...
    def __init__(self, name: str) -> None:
        ...
        try:
            ...
            self.boot_sector["Serial Number"] = hex(
                self.boot_sector["Serial Number"] & 0xFFFFFFFF
            )[2:].upper()
            self.boot_sector["Serial Number"] = (
                self.boot_sector["Serial Number"][:4]
                + "-"
                + self.boot_sector["Serial Number"][4:]
            )
    ...
    def __extract_boot_sector(self):
        ...
        self.boot_sector["Bytes Per Sector"] = int.from_bytes(
            self.boot_sector_raw[0xB:0xD], byteorder="little"
        )
        self.boot_sector["Sectors Per Cluster"] = int.from_bytes(
            self.boot_sector_raw[0xD:0xE], byteorder="little"
        )
        self.boot_sector["Reserved Sectors"] = int.from_bytes(
            self.boot_sector_raw[0xE:0x10], byteorder="little"
        )
        ...
        self.boot_sector["No. Sectors In Volume"] = int.from_bytes(
            self.boot_sector_raw[0x28:0x30], byteorder="little"
        )
        self.boot_sector["First Cluster of $MFT"] = int.from_bytes(
            self.boot_sector_raw[0x30:0x38], byteorder="little"
        )
        self.boot_sector["First Cluster of $MFTMirr"] = int.from_bytes(
            self.boot_sector_raw[0x38:0x40], byteorder="little"
        )
        self.boot_sector["Clusters Per File Record Segment"] = int.from_bytes(
            self.boot_sector_raw[0x40:0x41], byteorder="little", signed=True
        )
        self.boot_sector["MFT record size"] = 2 ** abs(
            self.boot_sector["Clusters Per File Record Segment"]
        )
        # self.boot_sector['Clusters Per Index Buffer'] = int.from_bytes(self.boot_sector_raw[0x44:0x45], byteorder='little')
        self.boot_sector["Serial Number"] = int.from_bytes(
            self.boot_sector_raw[0x48:0x50], byteorder="little"
        )
